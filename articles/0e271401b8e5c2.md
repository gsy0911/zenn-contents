---
title: "【NextAuth.js】cognitoでカスタムログインページを作成する"
emoji: "💫"
...
NextAuth.jsを使っていて、Providerにcognitoを使っている。
CognitoのデフォルトのUIはよくないので、カスタムでログイン画面を作成したい。
...
デフォルトのUIを使ったログイン方法は色々な記事でも紹介されていますが、
カスタムして`NextAuth.js`の`CredentialsProvider`を使ってログインすることをします。
この際に、フロントからサインインすることをします。

## 対象読者

- NextAuth.js or Auth.jsを使っている
- Cognitoを使っている
- CognitoのhostedUIは使いたくない
- [Mantine](https://mantine.dev/)を使っている
- ログインの機能はバックエンドではなくフロントエンド側で実施する
  - `Amplify.js`ではなく、`aws-sdk`を利用する
- `AWS CDK`を使っている

## 構成

フロントエンドはNextAuth.jsをLambdaにデプロイしてAPI Gateway経由でアクセスする形式をとります。
詳細は以下の記事をご覧ください。

https://tmokmss.hatenablog.com/entry/20221213/1670891305

かつログインを実行するのはNext.jsのAPI Routesの中で行うようにします。


## 料金

サーバーレスの構成なのでアクセス過多ではない限り料金はほとんどかからない想定です。
...
:::message
フロントエンドでログイン処理をしますが、Next.jsのAPI Routesを利用するため
クライアントシークレットが存在するプライベートクライアントで大丈夫です。
:::
...
`next-auth`ですでに定義されている`User`や`Session`の型を拡張します。
...
`next-auth`で大事なファイルについて説明します。
全てのコードを載せるのは難しいので適宜切り取りつつ説明していきます。
...
以下のsignIn関数が本記事でも大事な箇所になります。
`aws-sdk`を利用して、signInを実行しています。
...
export const authOptions: AuthOptions = {
  ...
  callbacks: {
    ...
    async jwt({ token, user, account }) {
      // Credentialsログインの場合: userに情報が付与されている
      if (account && user) {
...
以下のバリデーションを付与すると、デフォルトのサインイン画面に入力をした際に
...
:::message
今回はCognitoを作成するときに`self-signup`をfalseにしているため、
コンソールからユーザーを作成しています。
:::
...
ローカルのブラウザでテストする際にはゲストモードじゃないと動かないです。
