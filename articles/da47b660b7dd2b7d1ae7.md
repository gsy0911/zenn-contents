...
streamlitそのものには認証などの機能はないため、独自に実装したりする必要があります。
...
基本的にアクセスの頻度は低く、Fargateのリソースは`0.5vCPU, 256MiB`で計算しています。
...
Route53を利用している場合は、`your.domain.com`を管理しているホストゾーンに対して
...
AWSのコンソールから`Cognito`にアクセスして、
CDKで作成したCognitoにアクセスします。
...
アカウントが「有効」になり、
ステータスが「FORCE_CHANGE_PASSWORD」になったら準備完了です。

![](https://storage.googleapis.com/zenn-user-upload/4608b53402cf7e2a0a426a4e.png)

### 作成したページにアクセスする

アプリケーション作成のために用意したドメイン`streamlit.your.domain.com`にアクセスしてみます。
...
今回もCDKを利用して、streamlitにログイン機能を持たせてみました。
「 長期的な運用」というよりかは
「サクッと簡単に作って、すぐに削除する」ようなアプリケーションを想定しています。
