...
XStateを使って状態管理の部分をライブラリに則って、オレオレ実装にならないようにします。
...
ディレクトリ構成は以下のようになっています。👈がついているファイルが今回のサインアップで重要なファイルになっています。
...
全てクライアントサイドで閉じていることから各関数で文字列を返しています。こうすることで、useEffectで状態を監視して次の状態に移るタイミングを確認しています。サーバーサイドで閉じている場合には`next/navigation`の`redirect`関数でも良いかと思います。
