...
ManyToManyの`POST /api/users/{id}/books`を考えてみます。これは既存の`{id}`のユーザーが所持している書籍を登録するようなAPIを想定しています。先ほどの`User`クラスの修正の際に新規追加した処理です。
...
上記のエンドポイントに対して、次のリクエストを送信することを考えてみます。
