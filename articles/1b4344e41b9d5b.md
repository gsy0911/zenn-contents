...
* Terminal: WezTerm
* IDE: Intellij系（PyCharm, WebStorm, PhpStorm）とたまにVSCode
* Git Client: Fork
...
コーディングで主に使っているのはIntellij系のIDEです。Intellij系は、デフォルトで各種言語の補完が効き、プラグインを追加しなくても十分にストレスなくコーディングできるのが良い点だと思っています。プラグインを追加しないことで、新しいパソコンをセットアップしてもすぐに使い始めることができるのも良い点です。またどちらかというと、Intellijにプラグインの追加はしたくないです。プラグインの追加はGUIベースになるため、管理が煩雑になるのと異なる環境間での共有が難しいためです。もちろん、export機能を使えばいいのですが、設定を変更するたびにexportしてimportしてというのは煩雑な作業なためです。

VSCodeは、csvやjsonなどのテキストファイルを開いて確認・編集するときに使っています。VSCodeに関しても、同様の理由でプラグインの追加はしたくないです。プラグインの追加をしていないためコーディングには使えず、利用頻度は低めです。`Curosr`などの生成AIを組み込んだVSCodeベースのエディタも出てきていますが、使っていないです。



コーディング以外に利用しているのは、GitクライアントのForkとDatabaseクライアントのDataGripです。Forkに関しては、ターミナル上でTUIで操作できるLazygitを代替にする予定です。DataGripの代替はまだ見つかっていないです。一応、CUIでデータベースの操作もできるのですが流石に操作性が悪いので一旦はGUIベースのものを使う想定です。

IDE(Intellij)で使っている機能としては以下の通りです。
