...
次に`Caddyfile`を作成します。`{$CADDY_HOSTING_SITE_ADDRESS}`はCaddyが稼働しているサーバーのアドレス（＝ブラウザからアクセスするアドレス）を指定し、`${API_SERVER_ADDRESS}`はリバースプロキシで通す先を指定します。パス名が`/api/*`以下のリクエストをFastAPI側に流すようにしています。
