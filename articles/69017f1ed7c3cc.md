...
S3にリソースを探しに行く際に、存在しないパスをみに行こうとしてエラーになって今います。
そのため、CloudFrontとS3の間にLambda@Edgeが必要となります。
（正確にはオリジンリクエストなので、Lambda@EdgeはCloudFront内で閉じています）
...
S3のバケットポリシーにCloudFrontのOriginAccessIdentityを受けれる設定を追加します。
