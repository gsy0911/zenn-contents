...
:::message
プラグインマネージャーも新旧たくさんあって地味にどれを選択するか迷いました。加えてプラグインマネージャーごとにプラグインの追加の記述方法が異なるので、各種プラグインのREADMEにあるinstallのテキストのコピペができないことがあります。ただ、参考にはなるのでREADMEが全くの無意味というわけではないです。
:::
...
nvimを開きながら、ターミナルを開くことができます。設定を`direction = "float"`にしており、`<leader>tt`で前面にターミナルを出すことができます。`<leader>lg`で`lazygit`を出し、`q`でこの`lazygit`を閉じることができます。
...
neovimの見た目もpluginで変えることができます。「neovim colorscheme」などと検索するとトレンドなどを載せているサイトが出てきます。その中から以下の`catppuccin`を選びました。見た目が柔らかい感じがするので良さげです。本当はIntellijに近い`darcula`テーマを選びたかったのですが、あまり良さそうなのがなかったので一旦諦めました。
...
コミュニティ・技術書・記事もたくさんあるが、欲しい情報にアクセスするのがプログラミング言語に比べると慣れるまで大変という印象。
今なお活発である程度デファクトっぽいのもあるが、環境構築するのも大変。
luaという今までは触ったことのない言語を触る
最初に使うエディタではないと思う。特定の言語に精通し自分に合ったエディタを見つけた上で、そのエディタに使いたい機能が無い時に、neovimに移るのが良いと思いました。

メリット

補完の仕組みなどをしれて良い。
neovimが好きになる。
キーボードのみで完結できる（ただし、他のアプリを使っているので、完全に完結はしない）。
設定をdotfileで管理できる（他のIDEでもやろうと思えばできるが）。
vimが怖く無くなる（今まではnanoしか使っていなかった）

手放しに他人におすすめはできないけれど、私は好きです。
