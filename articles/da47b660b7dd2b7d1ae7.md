...
「アクセスの頻度は低い」・「Fargateのリソースは`0.5vCPU, 256MiB`」の想定で計算しています。
...
証明書の発行は、全てのサブドメインに適用できる`*.your.domain.com`を入力します。
...
`params.ts`に`vpcId`や作成したリソースの`ARN`を設定します。
注意点として、`domainPrefix`はユニークである必要があるので、
...
アプリケーションのために用意したドメイン`streamlit.your.domain.com`にアクセスしてみます。
