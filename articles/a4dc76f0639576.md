...
Raspberry Pi 5 で **Claude Code を常時起動**し、**外部から常時アクセス可能**にします。低消費電力で 24 時間つけっぱなしにできる Pi 5 は、Agent の常駐場所として手頃です。母艦の Mac とは切り離された専用機なので、多少強い権限を与えても隔離された中で完結します。

Claude Code を常駐させて外部からアクセスするだけなら、Mac でも問題なくできます。ただ、私の手元には MacBook Pro しかなく、Mac を閉じたりスリープに入ったりすると動作が不安定になってしまいます。常時起動させておける専用機が欲しかった、というのが Raspberry Pi を使う一番の理由です。
...
Raspberry Pi に新しいパッケージを追加したり、設定を変更したりする場合も Nix で更新します。この場合は SD カードの焼き直しではなく、`switch` という仕組みで更新内容を反映します。
...
:::message
Raspberry Pi の金額が高くなっていて驚きました。（もっと早くに買っておけばよかったと後悔しています……）
:::
...
Pi 5 に焼くのは `aarch64-linux` 向けのイメージですが、macOS 上の Nix はこれを直接ビルドできません。そのため Linux のビルダーとして Docker コンテナを使います（作り方は「[2. イメージのビルド](#2.-イメージのビルド)」で後述）。Apple Silicon なら arm64 ネイティブコンテナになるためエミュレーションが挟まらず、実機同等の速度で動きます。
...
- **擬似 TTY 経由で起動する**: TTY が無いと `claude` が非対話（`--print`）と誤判定して stdin を待ってしまうため、`util-linux` の `script` で擬似 TTY を割り当て、対話モードで起動させています。
- **時刻同期を待ってから起動する**: RTC の無い Pi は起動直後の時計がズレており、その状態で TLS 接続すると `CERT_NOT_YET_VALID` で失敗します。そこで `ExecStartPre` で timesyncd の同期完了（`/run/systemd/timesync/synchronized`）を待ってから `claude` を起動します。
- **`claude` は system プロファイルの実体を使う**: `/run/current-system/sw/bin/claude` を指すことで、対話ログイン時の `claude`（認証状態 `~/.claude`）と同一の実体にそろえています。
...
:::message
M1 MacBook Pro で 1 時間くらいかかりました。
:::
...
:::message
パスワード入力後、7 分くらいで終わりました。進捗は表示されないので、ちょっと不安になります。
:::
...
:::message
初回は HDMI で画面を繋ぎ、コンソールからログインするのも良いと思います。
:::
...
:::message
Raspberry Pi は 50 度以上になるとファンが回り始めます。
私はこのことを知らなかったので、起動後に公式ファンが回らず焦りました。
:::
...
:::message
`$ claude --remote-control` は対話型（`$ claude` と同じ）、`$ claude remote-control` は非対話型（サーバー型）で起動します。このオプションの差異に気がつくまで 3 日くらいかかりました。
:::
...
:::message
次のコマンドで Raspberry Pi の IP アドレスがわかります。
```shell: @ raspberrypi
$ hostname -I
```
:::
