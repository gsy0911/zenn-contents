...
ドキュメントを見ていて、単一のEntityを扱う場合であったり、複数のEntityでもGETに関する内容はパッと見ても結構わかります。ただ、POSTやPATCHなどの更新処理をする方法はパッとはわからないです。そのため、この記事でその辺りをまとめていきます。


## API Platformの印象

本題に入る前にAPI Platformの印象についてまとめます。触った時々に感じたことは以下のようになります。
...
API Platformを触っていると、Symfonyの`Serializer/Deserializer`がかなり重要でこれ以外はそこまで重要じゃないと思っています。API Platformを触る上で下の図のような印象を持ちつつ実装しています。最初見ただけでは以下の図の意味はわからないと思いますが、この記事を最後まで読んでもらえれば理解していただけるかと思います。また、私の理解が間違っていたらその点を指摘していただけると大変ありがたいです。
...
3. `User`クラスとManyToManyで紐づく`Book`クラスを考えて、その状態でGET/POSTを扱う。

（コードが進化していく図、関連の図があると理解ができそう）
