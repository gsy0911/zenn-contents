...
vimを使う際にはプラグインの追加が必須になるかと思います。複数のプラグインを導入・管理するために、まずはプラグインマネージャーを導入します。色々ありますが、`lazynvim`を導入しています。
...
ファイル検索などには`telescope.nvim`を利用しました。
