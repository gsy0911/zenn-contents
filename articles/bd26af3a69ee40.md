...
:::message
上記の例で`idToken`をキーとしてヘッダーに付与しているのは、`Authorization`キーがすでに利用されているためです。
これはIAM認証を使った場合に、`signedHeader`にそのキーがすでに存在しているためです。
Cognito認証通常の場合に`Authorization: ${idToken}`となっている場合には上記の処理は必要ないです。
:::

## 動作確認

動作確認をするために、S3バケットにユーザーの`IdentityId`ごとにファイルを設置します。
例では、`admin`と`user`という2つのユーザーが存在するため、
...
準備ができたら、フロントからファイルを読んで返すエンドポイントへアクセスします。
まずはadminユーザーからで、レスポンスに`{""data": "admin"}`という内容が見えます。

![](/images/nextjs_nextauthjs_cognito_3/nextjs_nextauthjs_cognito_3_4.png =600x)

userユーザーも同様に、レスポンスに`{""data": "user"}`という内容が見えます。

![](/images/nextjs_nextauthjs_cognito_3/nextjs_nextauthjs_cognito_3_3.png =600x)

次に、それぞれのアクセス制限が動いているところを確認します。
