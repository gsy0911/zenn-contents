...
フロントに認証をつけるの面倒じゃないですか？

Cognitoを利用していれば、Amplifyのライブラリがあって比較的楽になったかもです。
ただ、そうは言ってもフロントのアプリケーションのコードに組み込むのはちょっと面倒ですよね？

アクセスそのものを制限したい場合は、フロントのソースではなくアクセス時に制限をかけることもできます。
加えて、未認証の場合はコンテンツのダウンロードそのものを防ぐことができセキュリティ的にもかなり高く実装することができます。
...
今回利用するAWSのサービスは、主に以下の3つです。
これらのサービスを利用する場合は、サイトへのアクセスが100万回、ユーザー数も10万人程度にならないと課金が発生しないほどです。
そのため、ほぼ無料です。
...
`Route 53`でドメインを利用する必要があり、そこだけ料金がかかります。
取得するドメインによっても変わりますが、年額で1500円〜程度です。

## 構成

今回もCDKで作成していきます。
また、参考のGitHubで公開されている内容を流用することもあり、自分で`us-east-1`にデプロイしてます。
experimentalを利用すればより簡単にデプロイできることはできますが、現時点でできないのでそうしています。
...
詳細はそちらにも書いてあり、 AWSの記事を見るだけでもかなり参考になります。
...
|       ファイル名        |        使うところ        |    パラメータ設定タイミング     |
|:------------------:|:-------------------:|:-------------------:|
|     params.ts      |      CDKのデプロイ       |       デプロイする前       |
...
`subDomain1`と`subDomain2`の名前は今回は固定していますが、
好きな値を設定してもらって大丈夫です。
...
先ほど、Cognitoのデプロイが終わったので、`configuration.json`の編集を行います。
...
Lambda@Edgeの削除に時間がかかる

# おわりに

シンプルなCloudFrontもあるので比較してみてください。
