...
1つ目は、今なお活発である程度デファクトっぽいのもあるが、環境構築するのも大変な点です。

2つ目は、コミュニティ・技術書・記事もたくさんあるのですが、欲しい情報にアクセスするのがプログラミング言語に比べると慣れるまで大変という点です。

3つ目に、よく言われている操作に慣れるのが大変と言う点です。ただ、これは他の2つに比べると慣れの問題だけなので、触っていく上でとても大きなデメリットには感じないようになりました。

## メリット

1つ目は、設定をdotfileで管理できる点です（他のIDEでもやろうと思えばできるが）。

2つ目は、キーボードのみで完結できる（ただし、他のアプリを使っているので、完全に完結はしない）。

3つ目は、IDEの仕組みや細かいところを知れてよい点です。具体的な例が難しいのですが、「自作PCを作って普段使っているPCのパーツを意識するようになる」みたいな感覚が近い気がします。
