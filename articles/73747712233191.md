...
今までは`iTerm2`を使っており、大きな不満点はありませんでした。唯一挙げるとしたら、設定をVCSで管理することが難しい点です。設定をexportすることでVCSで管理することもできるみたいなのですが、正直export機能を使ったことがありませんでした。なんだかんだでGUIで管理するのは煩雑なためです。

`WezTerm`でいいなと思ったところは、Luaで設定を書くことができるところです。言語はなんでもいいのですが、VCSで設定を管理できるのがいいなと思いました。そこで本記事では、`WezTerm`の設定をしていきます。

# 設定を加えた`WezTerm`

`WezTerm`を記事や公式ドキュメントなどを参考にしつつ、以下のように作成しました。

![](/images/wezterm_tab/wezterm_tab_09.png =600x)

このターミナルは以下の事柄を前提としています。

* SSHなどはあまり多用していない
* 複数の異なるディレクトリを同時に開いて、並行で作業をする
  * `$ docker compose up`で複数コンテナを立ち上げつつ、同一のディレクトリで何かの作業をしたり、`$ docker compose exec ...`でコンテナ内に入って作業をする
  * ディレクトリAとディレクトリBで、並列で作業をする
* コードの編集はvimなどではなく、IDE（特にIntellij系）を想定している


# `WezTerm`の設定

それでは、設定をしていきます。何も設定を記述していない状態は以下のようになります。
...
次に、ターミナル部分を透過させて、ぼかしを入れます。`color_schema`は[ここ](https://wezfurlong.org/wezterm/colorschemes/index.html)にサンプルが載っています。特に、`Wombat`を選んだ理由はないです。ちなみに、`Nightly`を選ぶと`iTerm2`のデフォルトと同じ見た目になります。
...
次に、上で定義した内容を基にタブの見た目とテキストを変更します。タブがアクティブと非アクティブの状態で色が分かれるようになっています。加えて、非アクティブのタブでもどのディレクトリにいるのかがわかります。そのため、タブを選択するにしてもすぐに移動できます。
