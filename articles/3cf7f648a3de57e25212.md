...
そんな時に、CDKで作成したサービス群をSAMを利用してローカルで実行ができる！
これはアツいです！！
...
AWS CLIとCDKはあらかじめ入っていたので、`AWS SAM CLI - beta`のみbrewにて入れていきます。
...
そのため、Stack作成時にLambda関数に付与したS3のパスである環境変数を返すようにしています。
...
こちらも、問題なくいけました（返り値を載せる必要があるかは微妙ですが一応）。
