---
title: "IdTokenからIAM認証されたAPI Gatewayのエンドポイントへアクセスする"
emoji: "⚓"
...
特定の権限を持ったユーザーにのみ特定のAPIを叩けるようにしたい、ということはあると思います。

そんな時に役立つのが、Cognitoの`IDプール`です。
Cognitoの`ユーザープール`は認証を、`IDプール`は認可を担当しています。
詳しい概念などは以下を参照してください。
...
やりたいことは、とあるウェブアプリケーションにサインインしたユーザーごとに叩けるAPIを制限することです。
この時、API Gatewayを利用します。
...
CognitoのIDプールで、サインインユーザーがassumeできるロールを作成します。
この時、実行可能なAPI Gatewayをリソース単位で指定しておきます。
`accountId`と`apigwRestApiId`はStack内で付与します。
...
:::message
このIAMロールを作成するときに、IDプールのIDが必要になります。
このStackの作り方だと、2回のデプロイが必要です。
1回目はそのままデプロイして、2回目にIDプールのIDを指定してデプロイします。
:::


グループを作成し、上で作成したAdmin用のIAM Roleを指定します。
サインインユーザーが`admin`という名前を含むグループに属している場合に
IDプールを通してこのロールを付与するという設定です。
...
最後にIDプールを作成します。
上で作成した`roleMappingRule`を指定しています。
...
この時に、IDプールを使ってサインイン中のユーザーに紐付け可能なIAM Roleを取得します。
...
`GetCredentialsForIdentityCommandInput`の引数に`CustomRoleArn`を
指定してあげると、そのIAM Roleが付与可能ならそのIAM Roleを付与してくれます。
...
Cognitoの`IDプール`を使って、API Gatewayへの認可を付与してみました。
誰かの参考になれば幸いです。
