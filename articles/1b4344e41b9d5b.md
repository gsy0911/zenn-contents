...
コーディングで主に使っているのはIntellij系のIDEです。Intellij系は、デフォルトで各種言語の補完が効き、追加でプラグインを追加しなくても十分にストレスなくコーディングできるためです。プラグインを追加しないことで、新しいパソコンをセットアップしてもすぐに使い始めることができるのも良い点です。またどちらかというと、プラグインの追加はしたくないです。プラグインの追加はGUIベースになるため、管理が煩雑になるのと異なる環境間での共有が難しいためです。もちろん、export機能を使えばいいのですが、設定を変更するたびにexportしてimportしてというのは煩雑な作業なためです。

VSCodeは、csvやjsonなどのテキストファイルを開いて確認・編集するときに使っています。VSCodeに関しても、同様の理由でプラグインの追加はしたくないです。プラグインの追加をしないため、コーディングには使えるようにはならず、利用頻度は低めです。`Curosr`などの生成AIを組み込んだVSCodeベースのエディタも出てきていますが、使っていないです。
...
コーディング以外に利用しているのは、GitクライアントのForkとDatabaseクライアントのDataGripです。Forkに関しては、ターミナル上でTUIで操作できるLazygitを代替にする予定です。DataGripの代替はまだ見つかっていないです。

IDEで使っている機能としては以下の通りです。
