...
コーディングで主に使っているのはJetBrainsのIDEです。JetBrainsのIDEは、デフォルトで各種言語の補完が効き、プラグインを追加しなくてもストレスなくコーディングできるのが良い点だと思っています。プラグインを追加しないことで、新しいパソコンをセットアップしてもすぐに使い始めることができるのも良い点です。またどちらかというと、JetBrainsにプラグインの追加はしたくないです。プラグインの追加はGUIベースになるため、管理が煩雑になるのと異なる環境間での共有が難しいためです。もちろん、export機能を使えばいいのですが、設定を変更するたびにexportしてimportしてというのは煩雑な作業なためです。
...
コーディング以外のアプリケーションでは、GitクライアントのForkを利用しています。名前がGitのforkとかぶっているので検索するのが大変なのですが、見た目もシンプルでgit-flowの機能も最初から利用できて便利です。DatabaseクライアントにはJetBrainsのDataGripを利用しています。PyCharmやWebStormからでもほぼ同様のDatabaseクライアントが利用できるのですが、Database接続専用のクライアントがあるとシンプルになるのでDataGripを利用しています。
...
vimは今まで全く使ったことがありませんでした。vimは操作に慣れることが最初のハードルとしては高いのですが、`~/.config`以下で設定およびプラグインを全て楽に管理できるのが私にとって大きなメリットです。またターミナルだけで完結すること、ホームポジションからほとんど手を動かす必要がないため、どうしてもマウス操作が必要になるJetBrains系より早く操作ができるようになるかなと思っています。
