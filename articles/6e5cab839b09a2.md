...
フロントエンドからバックエンドのAPIをコールする時に型や補完がなくて辛いなと思いました。
良さげなライブラリは見つけられなかったので以下の記事を参考にリクエストなど型をつけつついい感じに参照できるようにします。
...
AppRouterは利用せずに、PagesRouterを利用します。
...
最後に、API Routesを利用してあらかじめ対応する検証用のAPIを作成します。
...
これでとりあえずの準備は完了です。これから型を定義していきます。
...
export type ReqQuery<T> = T extends ApiParams<infer I, any, unknown>
  ? I
  : never;
export type ReqBody<T> = T extends ApiParams<unknown, infer I, unknown>
  ? I
  : never;
export type ResBody<T> = T extends ApiParams<unknown, any, infer I>
  ? I
  : never;
```
...
ここでAPIのパラメータをそれぞれ定義できるようにするための型を定義しています。
...
import { ReqQuery, ReqBody, ResBody } from "@/common/types/api";
...
> = ReqQuery<Get<paths, `${Path}.${Method}`>>;

export type RequestData<
  Path extends UrlPaths,
  Method extends HttpMethods,
> = ReqBody<Get<paths, `${Path}.${Method}`>>;

export type ResponseData<
  Path extends UrlPaths,
  Method extends HttpMethods,
> = ResBody<Get<paths, `${Path}.${Method}`>>;
```
...
そして、`Path`と`Method`をキーに指定して、`paths`から対応する`interface`を取得するようにしています。
