...
コーディングで主に使っているのはIntellij系のIDEです。Intellij系は、デフォルトで各種言語の補完が効き、プラグインを追加しなくても十分にストレスなくコーディングできるためです。プラグインを追加しないことで、新しいパソコンをセットアップしてもすぐに使い始めることができるのも良い点です。またどちらかというと、Intellijにプラグインの追加はしたくないです。プラグインの追加はGUIベースになるため、管理が煩雑になるのと異なる環境間での共有が難しいためです。もちろん、export機能を使えばいいのですが、設定を変更するたびにexportしてimportしてというのは煩雑な作業なためです。
...
この状態で、`spec`にプラグインを追記していくことで複数のプラグインを管理・インストールできます。ただ、この状態を続けていると`init.lua`が非常に大きくなり管理しづらくなります。 以下のように追記することで、 **`lazy.nvim`ではプラグインを管理するファイルを分割することができます。** 
...
neovimはやはり、操作にある程度の慣れが必要になるのと、そもそも環境を作るのにも一苦労します。ただ、その分環境そのものに愛着がわくのともっと良い環境を求めるようになりました。

9月の末頃からneovimを触り始めて、3日ほどで慣れて環境も1日1~2時間程度で作っていき、トータル20~30時間ほどで最低限の機能は持たせられたかなという感じです。
